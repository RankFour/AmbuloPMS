...
let cashDetails = {
    title: "Cash Payment",
    description:
        "Visit the property management office during business hours to settle your balance in cash. Bring a valid ID and mention your lease reference when paying.",
...
let checkDetails = {
    title: "Check Payment",
    description:
        "Issue checks payable to Ambulo Property Management. Kindly write your full name, unit, and contact number at the back of the check. Post-dated checks are accepted for future-dated payments.",
...
let tutorialDetails = {
    ...
    uploadInstructions:
        "Upload a screenshot of the successful wallet transfer and keep the in-app transaction ID for our verification team.",
    walletInstructions:
        "After transferring, upload your payment proof and keep the reference number for verification.",
    bankInstructions:
        "After transferring, upload your payment proof and keep the reference number for verification.",
    additionalNotes:
    ...
    if (type === "wallet") {
        ...
        if (bankDetails.qrCode) {
            document.getElementById(
                "bankQrPreview"
            ).innerHTML = `<img src="${bankDetails.qrCode}" alt="QR Preview">`;
        }
    } else if (type === "cash") {
        ...
        document.getElementById("checkInstructions").value =
            checkDetails.instructions;
        ...
        document.getElementById("uploadInstructions").value =
            tutorialDetails.uploadInstructions;
        document.getElementById("walletInstructions").value =
            tutorialDetails.walletInstructions;
        document.getElementById("bankInstructions").value =
            tutorialDetails.bankInstructions;
...
function editWallet(id) {
    ...
    if (wallet.qrCode) {
        document.getElementById(
            "qrPreview"
        ).innerHTML = `<img src="${wallet.qrCode}" alt="QR Preview">`;
    }
}
...
function handleFormSubmit(e) {
    ...
    if (type === "wallet") {
        const walletData = {
            ...
            qrCode:
                document.getElementById("qrPreview").querySelector("img")?.src || "",
        };
        ...
        if (editId) {
            ...
            const newId =
                wallets.length > 0 ? Math.max(...wallets.map((w) => w.id)) + 1 : 1;
        ...
        bankDetails = {
            qrCode:
                document.getElementById("bankQrPreview").querySelector("img")?.src ||
                "",
...
function previewImage(input) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            document.getElementById(
                previewId
            ).innerHTML = `<img src="${e.target.result}" alt="QR Preview">`;
        };
