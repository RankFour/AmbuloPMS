  ...
  try {
    ...
    if (titleEl) {
      const token = (function getToken() {
        try {
          return (
            (typeof window !== "undefined" && window.JWT_TOKEN) ||
      ...
      const payload = (function decodeJwtPayloadLocal(t) {
        if (!t) return null;
        try {
          const parts = t.split(".");
      ...
      const getName = (pl) => {
        if (!pl) return "";
        const first = pl.first_name || pl.firstName || pl.first || "";
        return (first || "").trim();
      };
      const getRole = (pl) => {
        const r =
          pl && (pl.role || pl.user_role || pl.userRole || pl.scope || "");
        return typeof r === "string" && r ? r : "";
      };

      const name = getName(payload);
      let role = getRole(payload);
      if (role) role = String(role).toLowerCase();
      const fallback = role || "admin";
      titleEl.textContent = `Welcome, ${name || fallback}!`;
    }
  } catch { }
...
function renderSummaryKpis(isAdmin = true) {
  ...
  Array.from(row.querySelectorAll('.card[data-card-key^="kpi"]')).forEach(
    (el) => el.remove()
  );
  ...
  make(
    "kpiExpectedIncomeMonth",
    "Expected Income (This month)",
    "fas fa-dollar-sign",
    "/paymentAdmin.html",
    { tab: "charges", chargesStatus: "due" }
  );
  ...
  make(
    ...
    "fas fa-clock",
    "/paymentAdmin.html",
    { tab: "payments", view: "pending", pendingStatus: "Pending" }
  );
}
...
const DEFAULT_DASHBOARD_CONFIG = {
  metrics: [
    ...
    {
      key: "resolvedTicketsThisMonth",
      title: "Tickets Resolved (This Month)",
      icon: "fas fa-check-circle",
      visible: true,
      click: { href: "/maintenance.html" },
    },
    {
    ...
    },
    {
      ...
      click: {
        ...
        deepLink: { tab: "payments", view: "all" },
      },
    },
    {
      key: "tenants",
    ...
    },
    {
      ...
      click: {
        href: "/paymentAdmin.html",
        deepLink: { tab: "charges", chargesStatus: "overdue" },
      },
    },
    {
      key: "openTickets",
    ...
    const html = `
      <div class="outstanding-content" style="display:grid; gap:12px;">
        ...
        <div style="margin-top:8px; display:flex; gap:8px; justify-self:center; ">
          <a href="/paymentTenant.html" class="btn btn-info">Go to Payments</a>
        </div>
      </div>`;
...
function ensureMetricsRow() {
  ...
  const tasksCard = Array.from(grid.querySelectorAll(".card .card-title")).find(
    (t) => /tasks/i.test((t.textContent || "").trim())
  );
  if (
    tasksCard &&
    tasksCard.closest(".card") &&
    tasksCard.closest(".card").parentNode === grid
  ) {
...
export function renderDashboard(config) {
  const cfg =
    config && Array.isArray(config.metrics) ? config : DEFAULT_DASHBOARD_CONFIG;
  const container = ensureMetricsRow();
  if (!container) return;

  Array.from(container.querySelectorAll(".card[data-card-key]")).forEach((el) =>
    el.remove()
  );
  ...
  cfg.metrics
    ...
    .forEach((m) => {
      ...
      if (m.click && m.click.href) {
        card.style.cursor = "pointer";
        card.addEventListener("click", () =>
          navigate(m.click.href, m.click.deepLink)
        );
      }
...
export async function loadDashboardMetrics() {
  ...
  const collectedTotal = Number(
    paymentsStats?.totalCollected || paymentsStats?.total || 0
  );
  const pendingPaymentsCount = Number(paymentsStats?.pendingCount || 0);
  ...
  const openTickets = (Array.isArray(ticketsList) ? ticketsList : []).filter(
    (t) =>
      String(t.ticket_status || t.status || "").toLowerCase() !== "completed"
  ).length;
  ...
  setCardValueByKey(
    ...
    fmtInt(overdueCharges),
    totalCharges
      ? `${Math.round((overdueCharges / totalCharges) * 100)}% of charges`
      : ""
  );
  setCardValueByKey("openTickets", fmtInt(openTickets));
    ...
    if (card && recentPayments) {
      ...
      items.forEach((p) => {
        ...
        el.innerHTML = `
              <div class="transaction-icon"><i class="fas fa-receipt"></i></div>
          <div class="transaction-details">
            <h4>${(p.charge_description || p.description || "Payment")
            .toString()
            .slice(0, 48)}</h4>
            <p>${unit}${payer ? ` • ${payer}` : ""}</p>
            <small>${when ? new Date(when).toLocaleString() : ""}</small>
    ...
    const sparkSvg = `
      <svg width="100%" height="${sparkHeight}" viewBox="0 0 ${sparkWidth} ${sparkHeight}" xmlns="http://www.w3.org/2000/svg" style="max-width: 100%;">
        ...
        for (let i = 0; i <= ticks; i++) {
          ...
          const val = min + span * t;
          lines.push(
            `<line x1="${pad}" y1="${y.toFixed(1)}" x2="${sparkWidth - pad
            }" y2="${y.toFixed(1)}" stroke="#e5e7eb" stroke-width="1" />`
          );
          lines.push(
            `<text x="${pad - 6}" y="${(y + 4).toFixed(
              1
            )}" text-anchor="end" font-size="10" fill="#94a3b8">${fmtCurrency(
              val
            ).replace("₱", "")}</text>`
          );
        }
  ...
  try {
    const [expectedIncome, payDist, ticketsResp, leasesResp, contactsResp] =
      await Promise.all([
        getMonthlyExpectedIncome().catch(() => 0),
        getPaymentsDistribution().catch(() => ({ counts: {}, sums: {} })),
        apiGetTickets(1000).catch(() => ({ list: [], total: 0 })),
        ...
        getContactSubmissions(1, "pending").catch(() => ({
          list: [],
          total: 0,
        })),
      ]);
    ...
    try {
      const tickets = Array.isArray(ticketsResp?.list)
        ...
        : Array.isArray(ticketsResp?.tickets)
          ...
          : [];
      const isActive = (s) =>
        /PENDING|ASSIGNED|IN_PROGRESS/i.test(String(s || ""));
      const activeCount = tickets.filter((t) =>
        isActive(t.ticket_status)
      ).length;
      setCardValueByKey("openTickets", fmtInt(activeCount));
      ...
      setCardValueByKey(
        ...
        fmtInt(completedThisMonth.length)
      );

      const durations = completedThisMonth
        .map((t) => {
          const start = t.start_datetime
      ...
      const entries = Object.entries(countsByCat)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 7);
      const maxV = Math.max(1, ...entries.map(([, v]) => v));
      const totalTickets = entries.reduce((sum, [, v]) => sum + v, 0);
      ...
      setCardContentByKey(
        ...
        bars ||
        `<div style="color:#64748b; font-size:12px; padding: 40px 0; text-align: center;">No maintenance data</div>`
      );
    } catch (e) {
    ...
    try {
      const leases = Array.isArray(leasesResp?.list)
        ? leasesResp.list
        : Array.isArray(leasesResp?.leases)
          ? leasesResp.leases
          : [];
      const isActiveLease = (s) => /ACTIVE|PENDING/i.test(String(s || ""));
      const activeLeases = leases.filter((l) => isActiveLease(l.lease_status));
      ...
      activeLeases.forEach((l) => {
        if (l.property_id != null) activePropertyIds.add(String(l.property_id));
      });
      const occupiedProps = activePropertyIds.size;
      const denom = Math.max(1, Number(propertiesCount || 0));
      const occPct = Math.max(
        0,
        Math.min(100, Math.round((occupiedProps / denom) * 100))
      );
      ...
      setCardValueByKey(
        "kpiOccupancyRate",
        `${occPct}%`,
        `${fmtInt(occupiedProps)} of ${fmtInt(denom)} properties occupied`
      );
    } catch (e) {
    ...
    try {
      ...
      const unresolvedTickets = (
        Array.isArray(ticketsResp?.list)
          ? ticketsResp.list
          : Array.isArray(ticketsResp?.tickets)
            ? ticketsResp.tickets
            : []
      ).filter(
        (t) => !/COMPLETED|CANCELLED/i.test(String(t.ticket_status || ""))
      ).length;

      const newContacts = Number(
        (contactsResp?.stats &&
          (contactsResp.stats.pending ?? contactsResp.stats.unresponded)) ??
        contactsResp?.total ??
        0
      );
      ...
      const alertsHtml = `
        <div class="alerts-grid" style="display:grid; grid-template-columns: repeat(2, 1fr); gap: 14px;">
          <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 14px; background:#fff7ed; border:1px solid #fde68a; border-radius:10px;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-weight:800; color:#b45309;">${fmtInt(
        expiringSoon
      )}</span>
              <button type="button" data-action="goto-leases" class="btn-link">View</button>
            </div>
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 14px; background:#fef2f2; border:1px solid #fecaca; border-radius:10px;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-weight:800; color:#b91c1c;">${fmtInt(
        overdueCharges
      )}</span>
              <button type="button" data-action="goto-overdue" class="btn-link">Review</button>
            </div>
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 14px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:10px;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-weight:800; color:#1d4ed8;">${fmtInt(
        pendingPaymentsCount
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 14px; background:#ecfeff; border:1px solid #a5f3fc; border-radius:10px;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-weight:800; color:#0ea5e9;">${fmtInt(
        unresolvedTickets
      )}</span>
              <button type="button" data-action="goto-maint" class="btn-link">Open</button>
            </div>
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 14px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:10px; grid-column: span 2;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
              <span style="font-weight:800; color:#059669;">${fmtInt(
        newContacts
      )}</span>
              <button type="button" data-action="goto-contacts" class="btn-link">View</button>
            </div>
      ...
      try {
        ...
        if (card) {
          card
            .querySelector('[data-action="goto-leases"]')
          ...
          card
            ...
            ?.addEventListener("click", () => {
              try {
                localStorage.setItem(
                  "paymentAdminDeepLink",
                  JSON.stringify({ tab: "charges", chargesStatus: "overdue" })
                );
              } catch { }
              window.location.href = "/paymentAdmin.html";
            });
          card
            .querySelector('[data-action="goto-pending"]')
            ?.addEventListener("click", () => {
              try {
                localStorage.setItem(
                  "paymentAdminDeepLink",
          ...
          card
            .querySelector('[data-action="goto-maint"]')
            ?.addEventListener("click", () => {
              window.location.href = "/maintenance.html";
            });
          card
            .querySelector('[data-action="goto-contacts"]')
            ?.addEventListener("click", () => {
...
export function wireDashboardShortcuts() {
  const cards = Array.from(
    document.querySelectorAll(
      ".dashboard-grid .card, .transactions-section .card"
    )
  );
  ...
  const latePaymentsCard = Array.from(
    document.querySelectorAll(".transactions-section .card")
  ).find((c) =>
    /late\s*payments/i.test(
